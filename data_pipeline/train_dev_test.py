...
def train_dev_test(args):
    ...
    sampled_df = df.groupby('label').sample(n=minimum_count)
    test = sampled_df.groupby('label').sample(n=math.floor(0.1 * minimum_count))
    train_dev_df = sampled_df.drop(test.index)
    dev = train_dev_df.groupby('label').sample(n=math.floor(0.1 * minimum_count))
